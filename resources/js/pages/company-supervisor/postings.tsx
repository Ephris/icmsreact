...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Postings', href: '/company-supervisor/postings' },
];

export default function PostingsIndex({ postings, filters, success, error }: Props) {
  ...
  }, [success, error, postings, filters]);
  ...
  const handleDelete = (posting_id: number) => {
    ...
    router.delete(`/company-supervisor/postings/${posting_id}`, {
      onSuccess: () => {
      ...
      <div className="p-6 space-y-6">
        <Card className="shadow-lg border-none bg-white dark:bg-gray-800 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl">
          <CardHeader className="bg-gradient-to-r from-indigo-500 to-blue-600 dark:from-indigo-700 dark:to-blue-800 p-6 rounded-t-2xl transform transition-transform duration-300 hover:scale-[1.01]">
            <CardTitle className="text-2xl font-bold text-white tracking-tight">Manage Postings</CardTitle>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            ...
            <div className="flex flex-wrap gap-4 items-center">
              ...
              >
                <Link href="/company-supervisor/postings/create" aria-label="Create posting">
                  Create Posting
                </Link>
              </Button>
              <Button
              ...
              >
                <Link href="/company-supervisor/postings-trashed" aria-label="View trashed postings">
                  View Trashed
                </Link>
              </Button>
              <Button
                    ...
                    {postings.data.map((posting, index) => (
                      ...
                      >
                        ...
                        <TableCell className="text-gray-700 dark:text-gray-300">
                          {posting.skills_required && Array.isArray(posting.skills_required) && posting.skills_required.length > 0 
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    className="border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-white dark:bg-gray-900 rounded-full hover:bg-indigo-100 dark:hover:bg-indigo-900/50 hover:text-indigo-700 dark:hover:text-indigo-300 transition-all duration-300 transform hover:scale-105"
                                  >
                                    <Link href={`/company-supervisor/postings/${posting.posting_id}`} aria-label={`View posting ${posting.title}`}>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    className="border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-white dark:bg-gray-900 rounded-full hover:bg-indigo-100 dark:hover:bg-indigo-900/50 hover:text-indigo-700 dark:hover:text-indigo-300 transition-all duration-300 transform hover:scale-105"
                                  >
                                    <Link href={`/company-supervisor/postings/${posting.posting_id}/edit`} aria-label={`Edit posting ${posting.title}`}>
                  ...
                  <PaginationContent>
                    ...
                    {[...Array(postings.last_page)].map((_, i) => (
                      <PaginationItem key={i}>
                        <PaginationLink
                          href={`/company-supervisor/postings?page=${i + 1}&search=${encodeURIComponent(searchTerm)}&type=${typeFilter}&status=${statusFilter}&min_gpa=${minGpaFilter}&sort_by=${filters.sort_by}&sort_dir=${filters.sort_dir}`}
